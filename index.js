...
app.post("/run", function (req, res) {
  ...
  fs.writeFileSync(`${folder}/labyrinth.txt`, labyrinth);
...
`
  ...
  try {
    ...
    // Run the code
    programm_output = execSync(`cd ${folder} && docker run ${folder}`).toString();
 } catch (err) {
    console.log(err);
  }
  ...
  const [compile,output,stringpath]=programm_output.split("--------").map(x=>x.trim());
